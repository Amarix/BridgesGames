		...
		try{ // connect to the socket server
			...
			*/
			socket.on(io.socket.client.Socket.EVENT_CONNECT, new Emitter.Listener() {
				// EVENT_CONNECT events are emitted by the server whenever a socket connection is initiated and seen
				@Override
				public void call(Object... args) {
					/* Once socket connection is established, pass bridges credentials to set up channel access */
					socket.emit("credentials", "{\"user\":\"" + user + "\",\"assignment\":\""+ assignment.substring(0, assignment.indexOf("."))+ "\"}");
				    System.out.println("passing credentials to socket server...");
				 }
			}).on("announcement", new Emitter.Listener() {
				// announcement events are emitted by the server whenever another socket 
				//   subscribes to a channel on which this socket connection is listening
				@Override
				public void call(Object... args) {
				...
				// keydown events are emitted by the server whenever a key is pressed 
				//  in a channel on which this socket connection is listening 
				@Override
				public void call(Object... args) {
					JSONObject obj = (JSONObject)args[0];
				...
				// keyup events are emitted by the server whenever a key is unpressed 
				//  in a channel on which this socket connection is listening 
				@Override
				public void call(Object... args) {
					JSONObject obj = (JSONObject)args[0];
				...
				// EVENT_DISCONNECT events are emitted by the server if it crashes or terminates this socket's connection
				@Override
				public void call(Object... args) {
					try{
						System.out.println(args);
